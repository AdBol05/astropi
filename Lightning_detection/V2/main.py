...
import threading
import queue  # ? hopefully multithreading (1:CSV data, 2:image collection)
...
def read_data(data_file, spike_detected):  # data collection
    global i  # readings counter as a global variable
    # get position from timescale
    position = ISS.at(load.timescale().now()).subpoint()
    mag = sense.get_compass_raw()  # get magnetometer data
    time = datetime.now()
    time_eplased = time - startTime

    # debug
    print(f"reading data... used storage: {storage}/{max_storage} time elapsed: {time_eplased}")
    row = (i, time, position.latitude, position.longitude, position.elevation.km, mag.get("x"), mag.get("y"), mag.get("z"), spike_detected)  # assign data to row
...
def capture(cam, cnt):  # take a picture and add metadata to it (cam -> camera, cnt -> image counter)
    ...
    # convert coordinates to exif friendly format
    ...
    # capture camera and save the image
    cam.capture(f"{temporary_folder}/img_{cnt:03d}.jpg")
...
def data_collect(data_file, storage):
    for k in range(sequence):
        ...
        read_data(data_file, "-")
        # add data.csv file size of storage counter
        storage += os.path.getsize(data_file)
        sleep(1)  # wait one second

    if spike:  # if spike is not detected
        read_data(data_file, True)

    if spike:  # if spike is not detected
        read_data(data_file, False)
...
# debug
print(f"Program ended. All output files are located in {output_folder}")
